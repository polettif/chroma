...
<!-- [ and ] will be escaped by devtools::build_readme(), so we need to fix it manually afterwards... -->
```{r note, echo=FALSE, results='asis'}
cat('> [!NOTE]') 
```

> This fork of [jiho/chroma](https://github.com/jiho/chroma) contains changes to the documentation, examples and tests to pass `R CMD check`
